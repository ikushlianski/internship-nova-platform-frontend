...
const dataProvider: DataProvider<IUser | IStudent | IMentor | IPath | IClass> =
  {
    ...
    getOne: (resource, { id }) => {
      ...
      const item = (
        data as (IUser | IStudent | IMentor | IPath | IClass)[]
      ).find((item) => item.id === id);

      if (!item) {
        return Promise.reject(
          new Error(`Item with id ${id} not found in ${resource}`),
        );
      }
    ...
    getManyReference: <R extends IEntity>(
      resource: string,
      { target, id, pagination, sort }: GetManyReferenceParams<R>,
    ...
    update: (resource, { id, data }) => {
      ...
      const resourceData = dataStore[resource] as (
        | IUser
        | IStudent
        | IMentor
        | IPath
