...
export const handleGetRequest = <T>(collection: T[], map: Map<number, T>, entityName: string) => {
  showNotify(`${entityName} successfully loaded!`, { type: "success", autoHideDuration: NOTIFICATION_DISPLAY_DURATION.fast })
...
export const handleGetIdRequest = <T extends { id: number }>(params: { id?: string }, collection: T[], entityName: string) => {
  ...
  if (response) {
    showNotify(`${entityName} with ID ${id} successfully found!`, { type: "success", autoHideDuration: NOTIFICATION_DISPLAY_DURATION.long });
    return HttpResponse.json(response);
  } else {
    showNotify(`${entityName} with ID ${id} not found!`, { type: "error", autoHideDuration: NOTIFICATION_DISPLAY_DURATION.long });
...
export const handlePostRequest = async <T>(request: Request, collection: Map<number, T>, entityName: string) => {
  try {
    ...
    showNotify(`${entityName} successfully loaded!`, { type: "success", autoHideDuration: NOTIFICATION_DISPLAY_DURATION.long })
    return HttpResponse.json(newItem, { status: 201 })
  } catch (error) {
    showNotify("Failed to parse request body", { type: "error", autoHideDuration: NOTIFICATION_DISPLAY_DURATION.long })
...
export const handlePutRequest = async<T extends { id: number }>(params: { id?: string }, request: Request, collection: T[], map: Map<number, T>, entityName: string) => {
  try {
    ...
    showNotify(`${entityName} with ID ${id} successfully updated`, { type: "success", autoHideDuration: NOTIFICATION_DISPLAY_DURATION.long })
...
export const handleDeleteRequest = <T extends { id: number }>(params: { id?: string }, collection: T[], entityName: string) => {
  ...
  showNotify(`${entityName} with ID ${id} successfully deleted`, { type: "success", autoHideDuration: NOTIFICATION_DISPLAY_DURATION.long })
