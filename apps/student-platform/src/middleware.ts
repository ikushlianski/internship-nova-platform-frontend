...
export function middleware(req: NextRequest) {
  ...
  if (isPrivateRoute(req.nextUrl.pathname)) {
    if (tokenData) {
      console.log(`Entering private route ${req.nextUrl.pathname} with token data`, tokenData);

      return NextResponse.next();
    }

    console.log(`Cannot enter private route ${req.nextUrl.pathname} without a valid token`);
